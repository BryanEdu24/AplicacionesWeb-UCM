    ...
    console.log(varInput);
        $('#tablaTec').find('tr').each(i => { // Recorro la tabla por cada tr
            if (i === 0) {
                ...
                if (($($('#tablaTec').find('tr').eq(i)).find('td').eq(2).html().toLowerCase()).includes(varInput)) { // Devuelve lo que hay dentro del td (2 por la posicion del texto) y comprobamos con includes si contiene lo de varInput
                    // Nada
                }
                else {
